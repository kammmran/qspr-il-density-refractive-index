...
_UMAP_DIR = Path(__file__).resolve().parent.parent / "results" / "interactive_umap"
DEFAULT_TEMP_RANGE = (253.0, 573.0)
DEFAULT_PRESSURE_RANGE = (90.0, 110.0)
KNOWN_DATA_SOLVENTS = ["Pure ionic liquid", "water", "ethanol", "isopropanol", "Other..."]
...
def _run_prediction_with_status(
    ...
    status.update(label=f"Done: {len(result)} row(s) predicted.", state="complete")
    return result
...
def _build_pdf_report(result: pd.DataFrame, spec: ModelSpec) -> bytes:
    ...
    elements = [
        ...
        Spacer(1, 0.15 * inch),
        Paragraph(f"Rows predicted: {int(valid.sum())} / {len(result)}", styles["Normal"]),
    ]
    if valid.any():
        elements.append(
            Paragraph(
    ...
    if int(valid.sum()) >= 2:
        ...
        ax1.hist(result.loc[valid, "prediction_mean"], bins=min(20, max(3, int(valid.sum()))), color="#4C78A8")
        ...
        plt.close(fig1)
        buf1.seek(0)
        elements += [Image(buf1, width=chart_width, height=chart_height), Spacer(1, 0.15 * inch)]
        ...
        plt.close(fig2)
        buf2.seek(0)
        elements += [Image(buf2, width=chart_width, height=chart_height), Spacer(1, 0.2 * inch)]
    ...
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=letter, title=f"{spec.label} - QSPR predictions")
    doc.build(elements)
    buffer.seek(0)
...
def _umap_file_for_spec(spec: ModelSpec) -> Path | None:
    ...
    path = _UMAP_DIR / f"{property_token}_{spec.solvent.lower()}_neighbors5_dist01.html"
...
def _render_auto_train(df: pd.DataFrame, resolved_property: str, solvent_name: str | None) -> None:
    ...
    """
    st.info(f"No trained prediction model exists yet for '{resolved_property}'.")
    ...
    with col1:
        n_models = st.number_input("Ensemble members to train", min_value=1, max_value=5, value=5)
    with col2:
    ...
    if st.button("Train a new model on this data", type="primary"):
        ...
        status = st.status(f"Training a new model for '{resolved_property}'...", expanded=True)
        ...
        status.update(label=f"Done: trained {len(ensemble.models)} model(s).", state="complete")
        spec = ModelSpec(
            key="custom",
        ...
        st.session_state["trained_model"] = (ensemble, spec, mole_fraction_col, metrics, resolved_property, solvent_name)
    ...
    st.markdown("**Validation metrics** (group k-fold by IL SMILES, not this project's original tuning methodology)")
    ...
    if st.button(f"Run this newly trained model on the fetched data", type="primary"):
        ...
        if result is None:
            return
        st.dataframe(result)
        _render_download_buttons(result, spec, f"{resolved_property.lower().replace(' ', '_')}_{system_label}_predictions_from_custom_model")
...
def _run_data_mode(also_run_model: bool) -> None:
    ...
    if solvent_choice == "Pure ionic liquid":
        ...
        solvent_name = st.text_input("Solvent name (as it appears in ILThermo)", value="") or None
        st.caption("SMILES resolution is only reliable for water/ethanol/isopropanol -- other solvents are best-effort.")
    else:
        solvent_name = solvent_choice

    col1, col2 = st.columns(2)
    with col1:
        temp_min, temp_max = st.slider("Temperature (K) range", 200.0, 700.0, DEFAULT_TEMP_RANGE)
    with col2:
        limit_pressure = st.checkbox("Limit pressure range", value=True)
        pressure_range = None
        if limit_pressure:
            pressure_min, pressure_max = st.slider("Pressure (kPa) range", 0.0, 500.0, DEFAULT_PRESSURE_RANGE)
            pressure_range = (pressure_min, pressure_max)

    max_datasets = st.number_input("Max ILThermo datasets to download", min_value=1, value=30, step=5)

    if st.button("Fetch data", type="primary"):
        status = st.status(f"Fetching '{property_query}' data from ILThermo...", expanded=True)
        ...
        st.session_state["data_result"] = (df, resolved_property, solvent_name, property_query)
...
def _run_csv_mode(spec: ModelSpec) -> None:
    ...
    st.dataframe(result)
    _render_download_buttons(result, spec, f"{spec.property_name.lower().replace(' ', '_')}_{spec.solvent.replace(' ', '_')}_predictions")
    _render_report(result, spec)
    _render_umap_expander(spec)
...
def _run_single_entry_mode(spec: ModelSpec) -> None:
    ...
    if st.button("Predict", type="primary"):
        ...
        _render_download_buttons(result, spec, f"{spec.property_name.lower().replace(' ', '_')}_{spec.solvent.replace(' ', '_')}_prediction")
        _render_umap_expander(spec)
