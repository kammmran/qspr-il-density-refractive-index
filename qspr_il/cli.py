...
def _run_prediction_action(args: argparse.Namespace, spec: ModelSpec | None = None) -> int:
    ...
    spec = spec or (get_spec(args.model) if args.model else _interactive_choose_model())
...
def _prompt_range(label: str, default: tuple[float, float] | None) -> tuple[float, float] | None:
    ...
    raw = _prompt(f"{label} range (min-max, or 'none' for no limit)", default_str)
    ...
    try:
        ...
        print(f"Could not parse {raw!r} as 'min-max'; using default {default}.")
        return default
...
def _print_numbered_columns(items: list[str], columns: int = 3) -> None:
    ...
    col_widths = [
        max((len(labels[r + c * rows]) for r in range(rows) if r + c * rows < len(labels)), default=0) + 2
...
def _prompt_property() -> str:
    ...
    default_number = properties.index("density") + 1 if "density" in properties else 1
    while True:
        choice = _prompt("Property number (or type a property name)", str(default_number))
        if choice.isdigit():
            index = int(choice)
...
def _prompt_data_settings() -> dict:
    ...
    max_datasets_raw = _prompt("Max ILThermo datasets to download (blank = no limit)", "30")
...
def _run_data_action() -> tuple[pd.DataFrame, str | None, str | None, str] | None:
    ...
    if df.empty:
        ...
        print(f"Saved {len(df)} curated rows ({resolved_property}) to {output_path}")
...
def _offer_auto_train(df: pd.DataFrame, resolved_property: str, solvent_name: str | None) -> int:
    ...
    train_now = _prompt("Train a new model on this freshly fetched data now? [y/N]", "N")
    ...
    for m in metrics:
        print(f"  Model {m['model']}: RMSE={m['rmse']:.4g}, R2={m['r2']:.3f} (train={m['n_train']}, val={m['n_val']})")
    print(
...
def main(argv: list[str] | None = None) -> int:
    ...
    action = args.action or ("model" if args.model else None) or _prompt_action()
