...
default_model_dir = "density_isopropanol_ensemble_model"
...
def main(args):
    ...
    if args.mole_fraction_col is None or args.mole_fraction_col.strip() == "":
        ...
        print(
            f"Info: No mole fraction column specified. Using default: {default_mole_fraction}")
    ...
    if args.smiles_col not in data.columns or args.mole_fraction_col not in data.columns:
        print(
            f"Error: Input file must contain '{args.smiles_col}' and '{args.mole_fraction_col}' columns.")
        return
    ...
    data.rename(
        columns={args.mole_fraction_col: 'Mole_fraction'}, inplace=True)
    ...
    for i, (model, metadata) in enumerate(zip(models, model_metadata), 1):
        ...
        try:
            ...
            descriptor_array = np.hstack(
                (descriptor_array, temperature_array, mole_fraction_array))
        except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Apply the consensus model to predict density of IL in isopropanol.")
    parser.add_argument(
    ...
    parser.add_argument("--mole_fraction_col",
                        help="Name of the mole fraction column in the input CSV (default: Mole_fraction).")
    parser.add_argument(
        "--temp_col", help="Name of the temperature column in the input CSV (default: Temperature).")
    parser.add_argument(
        "--model_dir", help="Directory containing the ensemble of models and metadata (default: density_isopropanol_ensemble_model).")
    parser.add_argument(
